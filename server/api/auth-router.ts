...
export function setupAuthRouter() {
   ...
   * Admin access only
   */
  router.get('/api/auth/users', checkAuthenticated, checkAdmin, async (req, res) => {
    try {
   ...
   * Admin access only
   */
  router.post('/api/auth/setAdmin', checkAuthenticated, checkAdmin, async (req, res) => {
    try {
      ...
      if (typeof isAdmin !== 'boolean') {
        throw new AppError('isAdmin must be a boolean', 'validation/invalid-type');
      }
      
      // Set admin claim
      await setCustomUserClaims(uid, { admin: isAdmin });
      
      res.json({ success: true, message: `User ${uid} admin status set to ${isAdmin}` });
    } catch (error) {
      const errorResponse = handleError(error);
      res.status(errorResponse.statusCode).json(errorResponse);
    }
   ...
   * Admin access only
   */
  router.post('/api/auth/disableUser', checkAuthenticated, checkAdmin, async (req, res) => {
    try {
      ...
      if (typeof disabled !== 'boolean') {
        throw new AppError('disabled must be a boolean', 'validation/invalid-type');
      }
      ...
      res.json({ success: true, message: `User ${uid} disabled status set to ${disabled}` });
    } catch (error) {
      const errorResponse = handleError(error);
      res.status(errorResponse.statusCode).json(errorResponse);
    }
