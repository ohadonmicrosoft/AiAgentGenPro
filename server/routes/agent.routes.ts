 ...
 * @access Private
 */
router.post('/', authMiddleware, validateRequest(CreateAgentInput), async (req, res, next) => {
  try {
    const userId = req.user!.uid;
 ...
 * @access Private
 */
router.put('/:id', authMiddleware, validateRequest(UpdateAgentInput), async (req, res, next) => {
  try {
    const { id } = req.params;
    const userId = req.user!.uid;
    const updateData = req.body;
    
    const updatedAgent = await agentService.updateAgent(id, updateData, userId);
