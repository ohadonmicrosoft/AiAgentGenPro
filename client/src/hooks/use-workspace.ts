 ...
 */
async function updateWorkspace(id: string, data: Partial<Workspace>): Promise<Workspace> {
  try {
    const response = await fetch(`/api/workspaces/${id}`, {
      method: 'PATCH',
      headers: {
...
export function useWorkspace() {
  ...
  const { state, setCurrentWorkspace: setGlobalWorkspace, addNotification } = useGlobalState();
  ...
  return {
    ...
    isLoading: workspacesQuery.isLoading || (currentWorkspaceId ? currentWorkspaceQuery.isLoading : false),
    isError: workspacesQuery.isError || (currentWorkspaceId ? currentWorkspaceQuery.isError : false),
