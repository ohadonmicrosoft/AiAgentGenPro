...
>(
  ...
  const loadingToastConfig =
    typeof toastConfig.loading === "object"
      ? toastConfig.loading
      : defaultLoadingToast;
  ...
  const mutation = useReactQueryMutation<TData, TError, TVariables, TContext>({
    ...
    onSuccess: (data, variables, context) => {
      if (showSuccessToast) {
        toast.success(successToastConfig.title, successToastConfig.description);
      }
    ...
    onError: (error, variables, context) => {
      if (showErrorToast) {
        ...
        toast.error(formattedError.title, formattedError.description);
      }
