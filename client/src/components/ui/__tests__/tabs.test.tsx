...
describe('Tabs Component', () => {
  ...
  it('renders vertical tabs properly', () => {
    ...
    expect(screen.getByTestId('tabs-root')).toHaveAttribute('orientation', 'vertical');
    expect(screen.getByTestId('tabs-list')).toHaveAttribute('aria-orientation', 'vertical');
  ...
  it('supports fade-in animation for tab content', () => {
    render(
      <Tabs defaultValue="tab1">
        ...
        <TabsContent value="tab1">Tab 1 Content</TabsContent>
        <TabsContent value="tab2" fadeIn>Tab 2 Content</TabsContent>
      </Tabs>
    );

    // Second tab content should have animation class when fadeIn is true
    const tab2Content = screen.getByText('Tab 2 Content').closest('[data-testid="tabs-content"]');
    expect(tab2Content).toHaveClass('animate-fadeIn');
    
    // First tab content should not have animation class (fadeIn not set)
    const tab1Content = screen.getByText('Tab 1 Content').closest('[data-testid="tabs-content"]');
